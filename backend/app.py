...
def fill_date_range_with_transactions(df, from_date, to_date, account_number, sender_bank_name):
    import random
    """Fill complete date range with transactions and non-transaction days"""
    try:
        ...
        # Get last known balance before the period
        pre_period_data = df[pd.to_datetime(df['Transaction_Date']).dt.date < from_date.date()]
        if len(pre_period_data) > 0:
            last_balance = float(pre_period_data['Closing_Balance'].iloc[-1])
        else:
            # If no prior data, use opening balance of first transaction in the period
            period_data = df[
                (pd.to_datetime(df['Transaction_Date']).dt.date >= from_date.date()) &
                (pd.to_datetime(df['Transaction_Date']).dt.date <= to_date.date())
            ]
            if len(period_data) > 0:
                last_balance = float(period_data['Opening_Balance'].iloc[0])
            else:
                last_balance = 50000.0  # Default balance if no data
        
        # Create list to store all daily records
        all_daily_records = []
        
        # Process each date in range
        for single_date in date_range:
            date_only = single_date.date()
            
            # Get transactions for this specific date
            day_transactions = df[df['Transaction_Date_Only'] == date_only].copy()
            
            if len(day_transactions) > 0:
                # Sort transactions by time if time component exists
                if 'Transaction_Time' in day_transactions.columns:
                    day_transactions = day_transactions.sort_values('Transaction_Time')
                else:
                    day_transactions = day_transactions.sort_values('Transaction_Date')
                ...
                for _, transaction in day_transactions.iterrows():
                    ...
                    if transaction['Debit_Credit_Flag'] == 'DR':
                        last_balance -= float(transaction['Transaction_Amount'])
                    else:
                        last_balance += float(transaction['Transaction_Amount'])
                    ...
                    transaction_record = {
                        ...
                        'time': formatted_time,  # Random realistic time
                    ...
                    all_daily_records.append(transaction_record)
            else:
                # No transactions for this date - create null/NaN record
                # No transactions for this date - create null/NaN record
                import random
                ...
                no_transaction_record = {
                    'date': single_date.strftime('%d/%m/%Y'),
                    'time': formatted_time,  # Random time for non-transaction days
                    'description': 'NO TRANSACTIONS',
                    'reference_no': 'N/A',
                    'debit': None,  # None for non-transaction days
                    'credit': None,  # None for non-transaction days
                    'balance': round(last_balance, 2),  # Carry forward previous balance
                    'is_transaction_day': False
}
                all_daily_records.append(no_transaction_record)
...
def generate_bank_statement():
    """Generate bank statement with complete date range - including non-transaction days"""
    global processor
    ...
    try:
        ...
        # Fill complete date range with transactions and non-transaction days
        complete_transactions = fill_date_range_with_transactions(
        ...
        statement_data = {
            ...
            'summary': {
                ...
                'total_days': len(complete_transactions),
                'transaction_days': len([t for t in complete_transactions if t['is_transaction_day']]),
                'non_transaction_days': len([t for t in complete_transactions if not t['is_transaction_day']])
            },
            'transactions': complete_transactions
        }
        
        logger.info(f"Generated statement with {len(complete_transactions)} daily records")
        logger.info(f"Transaction days: {statement_data['summary']['transaction_days']}")
        logger.info(f"Non-transaction days: {statement_data['summary']['non_transaction_days']}")
        
        return jsonify({
            'success': True,
            'has_transactions': len(complete_transactions) > 0,
...
def download_statement_pdf():
    ...
    try:
        ...
        # Title and subtitle
        story.append(Paragraph(f"{statement_data['bank_info']['name']}", title_style))
        ...
        account_data = [
            ...
            [f"Account Holder: {statement_data['account_details']['account_holder']}", 
             f"From Date: {statement_data['statement_period']['from_date']}"],
            [f"Account Number: {statement_data['account_details']['account_number']}", 
             f"To Date: {statement_data['statement_period']['to_date']}"],
            [f"Account Type: {statement_data['account_details']['account_type']}", 
             f"Opening Balance: {format_currency_for_pdf(statement_data['statement_period']['opening_balance'])}"],
            [f"IFSC Code: {statement_data['account_details']['ifsc_code']}", 
             f"Closing Balance: {format_currency_for_pdf(statement_data['statement_period']['closing_balance'])}"],
            [f"Branch: {statement_data['account_details']['branch']}", '']
        ]
        ...
        summary_data = [
            [f"{format_currency_for_pdf(statement_data['summary']['total_credits'])}\nTotal Credits",
             f"{format_currency_for_pdf(statement_data['summary']['total_debits'])}\nTotal Debits",
             f"{statement_data['summary']['transaction_count']}\nTransactions",
             f"{format_currency_for_pdf(statement_data['summary']['net_change'])}\nNet Change",
             f"{statement_data['summary']['total_days']}\nTotal Days\n({statement_data['summary']['transaction_days']} active, {statement_data['summary']['non_transaction_days']} no activity)"]
        ]
        ...
        # Notice
        notice_text = f"Complete Date Range Statement: This statement shows every day from {statement_data['statement_period']['from_date']} to {statement_data['statement_period']['to_date']}, including {statement_data['summary']['non_transaction_days']} days with no banking activity."
        ...
        for transaction in statement_data['transactions']:
            if transaction['is_transaction_day']:
                ...
                debit_amount = "—"
                credit_amount = "—"
                balance_text = f"{format_currency_for_pdf(transaction['balance'])}\n(Carried Forward)"
        ...
        try:
            ...
            # Use HTML entities for ₹ symbol as fallback
            def format_currency_fallback(amount):
                if amount is None or amount == 0:
                    return "Rs. 0.00"  # Fallback to Rs.
                return f"Rs. {amount:,.2f}"
            
            # Rebuild content with fallback formatting
            story_fallback = []
            
            # Rebuild all content using fallback currency formatting
            # (Same structure but with format_currency_fallback instead of format_currency_for_pdf)
            
            # Title and subtitle
            story_fallback.append(Paragraph(f"{statement_data['bank_info']['name']}", title_style))
            story_fallback.append(Paragraph("Account Statement", subtitle_style))
            
            # Account Details with fallback formatting
            account_data_fallback = [
                ['Account Details', 'Statement Period'],
                [f"Account Holder: {statement_data['account_details']['account_holder']}", 
                 f"From Date: {statement_data['statement_period']['from_date']}"],
                [f"Account Number: {statement_data['account_details']['account_number']}", 
                 f"To Date: {statement_data['statement_period']['to_date']}"],
                [f"Account Type: {statement_data['account_details']['account_type']}", 
                 f"Opening Balance: {format_currency_fallback(statement_data['statement_period']['opening_balance'])}"],
                [f"IFSC Code: {statement_data['account_details']['ifsc_code']}", 
                 f"Closing Balance: {format_currency_fallback(statement_data['statement_period']['closing_balance'])}"],
                [f"Branch: {statement_data['account_details']['branch']}", '']
            ]
            
            account_table_fallback = Table(account_data_fallback, colWidths=[col_width, col_width])
            account_table_fallback.setStyle(TableStyle([
                ('BACKGROUND', (0, 0), (-1, 0), colors.lightgrey),
                ('TEXTCOLOR', (0, 0), (-1, -1), colors.black),
                ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
            ...
            story_fallback.append(account_table_fallback)
            story_fallback.append(Spacer(1, 16))
            
            # Build with fallback and try again
